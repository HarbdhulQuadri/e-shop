...
const createInvoice = async (data) => {
  ...
  const getAllInvoices = async () => {
    let query = {};
    let select = { projection: { _id: 0, invoiceID: 1, customerName: 1, customerEmail: 1, customerAddress: 1, invoiceDate: 1, lineItems: 1, total: 1 } };
    try {
      const result = await DbConnection.findAndSelectData(salesCollection, query, select);
